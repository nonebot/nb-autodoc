...
黑白名单和 Ref 处理:
    1. 为了避免子模块和包变量重名问题，默认输出所有模块，在构造时给予 skip_modules 变量，下划线开头模块会自动加入这个变量
...
from nb_autodoc import analyzer
...
class Module(Doc):
    ...
    To control module's documentable object, setting `__autodoc__` respected to:
        1. module-level dict variable
        2. key MUST be the current module's object's qualified name
            (even if it is force-exported to other modules)
        3. value True for whitelist (force-exported), False for blacklist
        4. value string will override target object's docstring
    ...
    def analyze(self) -> None:
        ...
        self.analyzer = analyzer.Analyzer(
            ...
            cast(str, self.obj.__file__),
        )

    def evaluate_string(self, expr: str) -> object:
